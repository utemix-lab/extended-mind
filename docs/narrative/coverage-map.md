# Coverage Map v1 (Narrative)

Общая нарративная картина: персонажи ↔ workbench’и ↔ домены.  
Ниже — карта перекрытия доменов workbench’ами и практиками.

## Персонажи → Workbench’и → Домены

### Вова
- **Workbench’и:** Музыка в целом, VSTablishment, Zucken‑Drücken, As if
- **Домены:**  
  - **Primary:** Music & Sound Culture  
  - **Partial:** Software & Digital Tools, Artificial Intelligence  
  - **Cross:** Visual Media (через As if)

### Петрова
- **Workbench’и:** Анимация +, Brand Direction, Circus Technologies
- **Домены:**  
  - **Primary:** Visual & Media Arts  
  - **Partial:** Design & Aesthetic Systems  
  - **Cross:** System-level (через Circus Technologies)

### Вася
- **Workbench’и:** Frame Grinder, As if
- **Домены:**  
  - **Primary:** Visual & Media Arts  
  - **Partial:** Interactive Systems  
  - **Cross:** Music & Sound (через As if)

### Нэй
- **Workbench’и:** Appearance Atelier
- **Домены:**  
  - **Primary:** Artificial Intelligence  
  - **Partial:** Visual & Media Arts

### CADрик
- **Workbench’и:** Solidol
- **Домены:**  
  - **Primary:** Architecture & Physical Realization  
  - **Partial:** Design & Aesthetic Systems

### Руна
- **Workbench’и:** Mojibake, Mind's Lorgnette, Dream Graph, Contracts
- **Домены:**  
  - **Primary:** Artificial Intelligence  
  - **Partial:** Software & Digital Tools, Knowledge Systems, Interactive Systems

### Hinto
- **Workbench’и:** Ji (ジ)
- **Домены:**  
  - **Primary:** Software & Digital Tools  
  - **Partial:** Artificial Intelligence

### Ancy
- **Workbench’и:** Darwin’s Blind Spot, Evoquant, Mind's Lorgnette
- **Домены:**  
  - **Primary:** Knowledge Systems & Cognitive Architectures  
  - **Partial:** Formal Systems

### Геймыч
- **Workbench’и:** Playfield, Word Machine
- **Домены:**  
  - **Primary:** Interactive Systems & Game Culture  
  - **Partial:** Knowledge Systems (формальные машины)

### Дизи
- **Workbench’и:** Homo Perceptivus, Киберантика
- **Домены:**  
  - **Primary:** Design & Aesthetic Systems  
  - **Partial:** Visual & Media Arts

### ИИ
- **Workbench’и:** Iron Fairy Tales
- **Домены:**  
  - **Primary:** Artificial Intelligence (industry mirror)  
  - **Cross:** System voice

### Автор
- **Workbench’и:** Extended Mind
- **Домены:**  
  - **Primary:** System-level  
  - **Cross:** Physical Realization (через CADрик)

---

## Карта перекрытия доменов (Workbench’и + практики)

**Artificial Intelligence & Computational Models**  
Workbench’и: Iron Fairy Tales, Appearance Atelier, Mojibake, Ji, Zucken‑Drücken, Solidol  
Практики: system-thinking, research-thinking, ecosystem-thinking

**Software & Digital Tools**  
Workbench’и: VSTablishment, Frame Grinder, Mojibake, Dream Graph, Contracts, Solidol, Ji  
Практики: possibility-navigation, interactive-thinking, system-thinking

**Visual & Media Arts**  
Workbench’и: Анимация +, Appearance Atelier, Frame Grinder, Homo Perceptivus, As if  
Практики: direction, visual-thinking, curation

**Music & Sound Culture**  
Workbench’и: Музыка в целом, VSTablishment, Zucken‑Drücken, As if  
Практики: sound-thinking, curation, direction

**Design & Aesthetic Systems**  
Workbench’и: Homo Perceptivus, Киберантика, Brand Direction  
Практики: visual-thinking, branding, systems-design

**Knowledge Systems & Cognitive Architectures**  
Workbench’и: Darwin’s Blind Spot, Evoquant, Mojibake, Word Machine, Mind's Lorgnette  
Практики: reflection-fixation, research-thinking, system-thinking

**Interactive Systems & Game Culture**  
Workbench’и: Playfield, Frame Grinder, Dream Graph  
Практики: interactive-thinking, system-thinking, possibility-navigation

**Architecture & Physical Realization**  
Workbench’и: Solidol  
Практики: architectural-design, systems-design, visual-thinking

---

## Notes
- Домены — не разделы сайта.  
- Персонажи — не аватары.  
- Workbench’и — не фичи.  
Это разные входы в один и тот же мир.
